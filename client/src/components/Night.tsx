...
export default function Night({ onToast }: Props) {
  ...
  const [emotions,   setEmotions]   = useState<string[]>(cached.emotions || [])
  const [tomorrow,   setTomorrow]   = useState<string>(cached.tomorrow_action)
  const [gratitude, setGratitude] = useState<string>(() => {
    try { return localStorage.getItem('decode_gratitude_' + todayStr()) ?? '' } catch { return '' }
  })
  ...
  // Track whether user has interacted with the form — prevents BQ overwriting input
  const userTouched = useRef(false)

  // Auto-save draft to localStorage on every change (prevents data loss on tab switch / midnight remount)
  useEffect(() => {
    if (!userTouched.current) return
    saveNightCache({ outcome, focus_level: focus, mood_level: mood, emotions, tomorrow_action: tomorrow, reflection })
  }, [outcome, focus, mood, emotions, tomorrow, reflection])
  ...
  useEffect(() => {
    api.getToday()
      .then(row => {
        ...
        // If night log already saved to BQ, lock the form (always overwrite — this is authoritative)
        if (row.day_outcome) {
          setOutcome(row.day_outcome as any)
          if (row.focus_level)     setFocus(row.focus_level)
          if (row.mood_level)      setMood(row.mood_level)
          if (row.emotions)        setEmotions(row.emotions.split(',').filter(Boolean))
          if (row.tomorrow_action) setTomorrow(row.tomorrow_action)
          if (row.reflection)      setReflection(row.reflection)
          setAlreadySaved(true)
          setSaved(true)
        } else if (!userTouched.current && !cached.outcome) {
          // Only populate from BQ if user hasn't started filling in and local cache is empty
          if (row.focus_level)  setFocus(row.focus_level)
          if (row.mood_level)   setMood(row.mood_level)
        }
      })
  ...
  const handleSave = async () => {
    ...
    const payload: LogPayload = {
      ...
      focus_level:   focus,
      mood_level:    mood,
      emotions:      emotions.length > 0 ? emotions.join(',') : null,
      day_outcome:   outcome,
      tomorrow_action: tomorrow.trim(),
      reflection:    reflection.trim() || null,
    }
    saveNightCache({ outcome, focus_level: focus, mood_level: mood, emotions, tomorrow_action: tomorrow.trim(), reflection: reflection.trim() })
    setLoading(true)
    try {
  ...
  return (
    <div>
      ...
      <div className="page-intro">
        ...
        <div className="page-intro-sub">Decode how you felt today</div>
      </div>
        ...
        <div className="emotion-picker">
          {EMOTION_CATEGORIES.map(cat => (
            <div key={cat.id} className="emotion-category">
              <div className="emotion-cat-label" style={{ color: cat.color }}>{cat.label}</div>
              <div className="emotion-tags">
                {EMOTIONS.filter(e => e.category === cat.id).map(em => {
                  const selected = emotions.includes(em.id)
                  return (
                    <button
                      key={em.id}
                      className={`emotion-tag ${selected ? 'emotion-tag-active' : ''}`}
                      style={{
                        '--emotion-color': em.color,
                        borderColor: selected ? em.color : undefined,
                        background: selected ? `color-mix(in oklch, ${em.color} 15%, transparent)` : undefined,
                      } as React.CSSProperties}
                      onClick={() => toggleEmotion(em.id)}
                      disabled={alreadySaved || (!selected && emotions.length >= 3)}
                    >
                      <span className="emotion-tag-emoji">{em.emoji}</span>
                      <span className="emotion-tag-label" style={{ color: selected ? em.color : undefined }}>{em.label}</span>
                    </button>
                  )
                })}
              </div>
          ...
          return (
            <div className="slider-block" key={cfg.key}>
              ...
              <input
                ...
                onChange={e => { if (!alreadySaved) { userTouched.current = true; set(Number(e.target.value)); setSaved(false) } }}
      ...
      <div className="card">
        ...
        <textarea
          ...
          rows={2}
          value={tomorrow}
          onChange={e => { if (!alreadySaved) { userTouched.current = true; setTomorrow(e.target.value); setErrors(p => ({ ...p, tomorrow: false })); setSaved(false) } }}
      ...
      <div className="card intention-card">
        ...
        <div className="intention-prompt">
          ...
          <span className="intention-prompt-text">What went well today? What are you grateful for?</span>
        </div>
        <textarea
          ...
          onChange={e => {
            if (!alreadySaved) {
              ...
              try { localStorage.setItem('decode_gratitude_' + todayStr(), e.target.value) } catch {}
            }
          }}
          placeholder='e.g. "Had a great walk outside, finished the presentation early"'
          disabled={alreadySaved}
        />
      </div>

      {/* Step 6 — Reflection with prompt */}
      <div className="card">
        <div className="card-label">
          Reflection
          <span className="optional-badge">optional</span>
        ...
        <div className="reflection-prompt">
          <span className="reflection-prompt-icon">🪞</span>
          <span className="reflection-prompt-text">{todayPrompt.question}</span>
        </div>
        <textarea
          className="field-textarea"
          rows={3}
          value={reflection}
          onChange={e => { if (!alreadySaved) { userTouched.current = true; setReflection(e.target.value); setSaved(false) } }}
